...
from src.transform.chart_4_gap_shapes import build_chart_4_plot_table
from src.transform.constants import MEDIUM_TERM_TIME_WINDOW, SHORT_TERM_TIME_WINDOW


def create_chart_4(chart_table: pd.DataFrame) -> Figure:
    apply_chart_style()
    plot_table = build_chart_4_plot_table(chart_table)
    ...
    axis.set_xlim(*CHART_4_X_AXIS.limits)
    axis.set_xticks(
        np.array(
            [
                CHART_4_X_AXIS.minimum,
                *np.arange(0, CHART_4_X_AXIS.maximum + 1, CHART_4_X_AXIS.tick_step),
            ]
        )
    )
    axis.grid(
